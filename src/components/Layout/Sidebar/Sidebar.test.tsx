...
describe("Sidebar", () => {
  ...
  it("renders all nav items", () => {
    render(<Sidebar currentView="Board" repoId="repo-1" />);
  ...
  it('shows "Idle" when agentActive is false', () => {
    render(<Sidebar currentView="Board" repoId="repo-1" agentActive={false} />);
    expect(screen.getByText("Idle")).toBeInTheDocument();
  });

  it('shows "Active" when agentActive is true', () => {
    render(<Sidebar currentView="Board" repoId="repo-1" agentActive={true} />);
    expect(screen.getByText("Active")).toBeInTheDocument();
  });

  it("navigates to board when Board is clicked", async () => {
    render(<Sidebar currentView="Tasks" repoId="repo-1" />);
  ...
  it("does not navigate when repoId is undefined and a main nav item is clicked", async () => {
