...
describe("RepoSwitcher", () => {
  ...
  it("shows the active repo name in the trigger", () => {
    render(<RepoSwitcher activeRepoId="repo-1" />);
    expect(screen.getByText("Frontend")).toBeInTheDocument();
  });

  it("shows 'Select repo' when activeRepoId does not match any repo", () => {
    render(<RepoSwitcher activeRepoId="unknown" />);
  ...
  it("navigates to a repo's board when its name is selected in the dropdown", async () => {
    ...
    expect(mockPush).toHaveBeenCalledWith("/repos/repo-2/board");
  });
