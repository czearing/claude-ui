...
describe("SpecEditor", () => {
  ...
  it("renders nothing when task is null", () => {
    const { container } = render(
      <SpecEditor repo="repo-1" task={null} onClose={jest.fn()} />,
    );
  ...
  it("shows an h1 title for non-Backlog tasks", () => {
    render(
      <SpecEditor repo="repo-1" task={inProgressTask} onClose={jest.fn()} />,
    );
    expect(
  ...
  it("shows Agent Notes for Review tasks", () => {
    render(<SpecEditor repo="repo-1" task={reviewTask} onClose={jest.fn()} />);
  ...
  it("calls onClose when the Close button is clicked", async () => {
    ...
    render(<SpecEditor repo="repo-1" task={backlogTask} onClose={onClose} />);
  ...
  it("does not save spec when content matches the already-saved value", async () => {
    ...
    render(
      <SpecEditor repo="repo-1" task={taskWithSpec} onClose={jest.fn()} />,
    );
    await userEvent.click(screen.getByTestId("editor-change-trigger"));
